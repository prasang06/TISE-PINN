...
def psi_theta_norm(params, x):
    psi_vals = psi_normalized(params, x_data)
    idx = jnp.argmin(jnp.abs(x_range - x))
    return psi_vals[idx]

def psi_theta(params, x):
    return psi_raw(params, x) / psi_norm_constant(params)



psi_x = jax.grad(psi_theta_norm, argnums=1)
...
def total_loss(params):
    return lambda_pde*pde_loss(params) + E_loss(params) # + lambda_bc*boundarycond_loss(params)  + lambda_norm*normal_loss(params)
...
def update(params, learning_rate):
    ...
    new_parameters = [p-learning_rate*g for p,g in zip(params, grads)]
